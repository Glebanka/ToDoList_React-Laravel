...
const Task: React.FC<TaskProps> = ({task, deleteFunction}) => {
  return(
  <div className='rounded-2xl border-solid border w-11/12 flex flex-col p-4 gap-3'>
    <strong className='text-3xl'>{task.title}</strong>
    <p className='whiteColor'>{task.description}</p> 
    
    <div className='self-end'>
      <button onClick={() => deleteFunction(task.id)}>
